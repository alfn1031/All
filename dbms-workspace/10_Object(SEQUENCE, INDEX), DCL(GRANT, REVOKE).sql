/*
 *  < SEQUENCE ( 시퀀스 ) >
 * 자동으로 번호를 만들어주는 객채 -< 채번
 * 주로 PRIMARY KET 값으 ㄹ자동으로 생성 할 때 사용
 * 
 ...
 * 뷰 : VW_뷰명
 * 시퀀스 : SEQ_시퀀스명
 * 인덱스 : IDX_인덱스명 ( NEXTVAL 한 번 이상 호출 후 사용가능)
 */
...
-- < INDEX > 
/*
 * 데이터 검색 속도를 향상시키기 위한 객체(책의 차례 목차 색인 이거랑 같음)
 * 
 * - PRIMARY KEY, UNIQUE 제약조건을 달아주는 컬럼은 자동으로 인덱스 생성됨
 * - 데이터 변경(INSERT, UPDATE, DELETE)이 빈번한 컬럼은 인덱스 비추천
 */

CREATE INDEX IDX_ANIMAL_NAME ON ANIMAL(ANIMAL_NAMEALTER ; -- 동물이름으로 속도 향상)
...
-- 특정 테이블 조회 권한 부여
GRANT SELECT ON C##MS.ANIMAL TO C##STUDENT;

-- ROLL(역할) 부여 : 여러 권한을 묶어놓은 것
