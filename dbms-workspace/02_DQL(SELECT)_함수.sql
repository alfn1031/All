/*
	< 함수 FUNCTION >
	자바로 따지면 자바 API메소드 같은 개념
	함수 호출시 전달된 값을 받아서 계산된 결과를 반환
	
	- 단일행정함수 (이런게 있구나~~) : N개의 값을 읽어서 N개의 결과를 반환
		(매 행마다 함수 실행)
	- 그룹함수 (그냥 외우세요~) : N개의 값을 읽어서 1개의 결과를 반환
		(하나의 그룹별로 함수 실행)
		
		단일 행함수 / 그룹함수는 일반적으로 함께 사용하지 않음
...
/*
 * SUBSTER(문자열, 시작위치, 추출할문자수)
 * 문자열에서 특정 문자열을 추출해서 반환
 */
SELECT * FROM KEEPER;
SELECT 
		KEEPER_NAME
	  , SUBSTER(KEEPER_NAME, 1, 1) AS "이름 첫 글자"
  FROM
  		KEEPER;
 ...
 * 
 * NVL (컬럼, NULL일때 반환할 값)
 * NVL2 (컬럼, NULL이 아닐 떄 값, NULL일때 값)
 */
SELECT 
		ANIMAL_NAME
	  , NVL(TO)CHAR(ARRIVAL_DATE, 'YYYY-MM-DD'), '동뭉원에서 태어남')
  FROM
  		ANIMAL;
...
SELECT 
		ANIMAL_NAME
	  , KEEPER_ID(KEEPER_ID, '배정완료', '미지정') "사육사 배정여부"
  FROM 
  		ANIMAL;
  -------------------------------------------
-- < 선택 함수 > 
...
-- 전체 동물수
SELECT COUT(*) FROM ANIMAL;

-- 평균 체중
SELECT AVG(WEIGHT_KG) FROM ANIMAL;
SELECT LENGHT(ANIMAL_NAME) FROM ANIMAL;
...
-- COUNT(컬럼) : NULL값을 제외하고 카운트
SELECT
	   COUNT(KEEPER_ID) AS "사육사 배정된 동물 수"
  FROM
	   ANIMAL;
